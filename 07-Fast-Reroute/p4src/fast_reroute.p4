...
control MyIngress(inout headers hdr,
    ...
    action get_timestamp_for_port(){
        // TODO: Read the timestamp for the port index that is written on the heartbeat header.
    }

    action set_timestamp_for_port(){
        // TODO: Set the timestamp for the specific port that the heartbeat message is received.
    }

    action send_heartbeat(){
        // TODO: Send the heartbeat message from the correct port and ensure that the switch receiving it can distinguish it from its own heartbeat messages.
    }
    ...
    apply {
        ...
        if (hdr.heartbeat.isValid()){
            ...
            if (hdr.heartbeat.from_cp == 1){
                ...
                // TODO: If it has been at least THRESHOLD since the last received heartbeat, set the linkState to down.
                // TODO: Then, clone the heartbeat to the controller while forwarding the original heartbeat to its destination.
        ...
        if (hdr.ipv4.isValid()){
            ...
            // TODO: Switch to the backup next hop if the link state is 1.
